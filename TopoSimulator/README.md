...
The default best-path order is highest local preference, shortest AS_PATH, lowest MED, EBGP over IBGP, then current best path over a new equal route. NEXT_HOP and learned-from lexicographic tie-breakers are used only when there is no still-valid current best path with the same primary preference.
...
Neighbor entries can include `"mrai_ms"` to enforce a per-neighbor MRAI for advertised UPDATE messages. `mrai_ms=0` disables MRAI. Withdrawal-only UPDATEs bypass MRAI and are sent immediately; pending advertised UPDATEs to the same neighbor share one transport flush at the next MRAI slot.
