...
public:
    ...
    [[nodiscard]] std::optional<NeighborConfig> neighbor(const std::string& peer_id) const;
...
protected:
    ...
    virtual RouteEntry transformRouteForPeer(const RouteEntry& route,
                                             const NeighborConfig& to_peer) const;
    virtual std::optional<RouteEntry> selectBestRoute(
...
private:
    ...
    void disseminateChangedRoutes(const std::map<std::string, std::optional<RouteEntry>>& changes);
    [[nodiscard]] std::vector<RouteEntry> candidatesForPrefix(const std::string& prefix) const;
    [[nodiscard]] bool shouldReflectIbgpRoute(const RouteEntry& route,
                                              const NeighborConfig& to_peer) const;
