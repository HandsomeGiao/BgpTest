...
BmpCollector::BmpCollector(std::filesystem::path log_file)
    ...
    if (!out_) {
        throw std::runtime_error("Unable to open BMP collector log: " + log_file_.string());
    }
}
...
void BmpCollector::recordReceive(const std::string& router_id, const BgpMessage& message) {
    writeLine({
...
const std::filesystem::path& BmpCollector::logFile() const {
    return log_file_;
}
