...
TopoManager::TopoManager(TopologyConfig config)
    ...
    const auto worker_count = defaultWorkerCount(config_.simulation.worker_threads);
...
TopologyConfig TopoManager::loadTopology(const std::filesystem::path& topology_file) {
    std::ifstream in(topology_file);
    if (!in) {
        throw std::runtime_error("Unable to open topology file: " + topology_file.string());
    }
    nlohmann::json j;
    in >> j;
...
void TopoManager::sendMessage(const std::string& from, const std::string& to, BgpMessage message) {
    ...
    pool_->enqueue([destination = std::move(destination),
                    collector,
                    delay_ms,
...
void TopoManager::setLinkState(const std::string& a, const std::string& b, bool enabled) {
    ...
    bmp_->recordTopologyEvent(enabled ? "link_up" : "link_down", {{"a", a}, {"b", b}});
    if (enabled) {
...
void TopoManager::setRouterState(const std::string& router_id, bool enabled) {
    ...
    bmp_->recordTopologyEvent(enabled ? "router_up" : "router_down", {{"router", router_id}});
    if (enabled) {
        router->start();
        for (const auto& peer : peers) {
            if (auto peer_router = routers_.find(peer); peer_router != routers_.end()) {
...
void TopoManager::originatePrefix(const std::string& router_id, const std::string& prefix) {
    std::shared_ptr<BgpRouter> router;
    {
        std::lock_guard lock(mutex_);
        router = routers_.at(router_id);
    }
    bmp_->recordTopologyEvent("manual_advertise", {{"router", router_id}, {"prefix", prefix}});
    router->originatePrefix(prefix);
}

void TopoManager::withdrawPrefix(const std::string& router_id, const std::string& prefix) {
    std::shared_ptr<BgpRouter> router;
    {
        std::lock_guard lock(mutex_);
        router = routers_.at(router_id);
    }
    bmp_->recordTopologyEvent("manual_withdraw", {{"router", router_id}, {"prefix", prefix}});
    router->withdrawLocalPrefix(prefix);
}

bool TopoManager::waitForConvergence(std::chrono::milliseconds timeout) {
    return pool_->waitForIdle(timeout, std::chrono::milliseconds(config_.simulation.convergence_quiet_ms));
}
...
void TopoManager::normalizeNeighborsFromLinks() {
    auto find_config = [this](const std::string& id) -> const RouterConfig& {
        const auto it = std::find_if(config_.routers.begin(), config_.routers.end(),
    ...
    for (const auto& link : config_.links) {
        ...
        if (!a_router->neighbor(link.b)) {
            a_router->addOrUpdateNeighbor({
                ...
                .session_type = a_config.asn == b_config.asn ? SessionType::Ibgp : SessionType::Ebgp,
                .rr_client = false,
                .enabled = true,
            });
        }
...
std::optional<TopoManager::LinkRuntime> TopoManager::linkFor(const std::string& a,
...
std::filesystem::path TopoManager::makeRunDirectory() const {
    std::filesystem::path base = config_.simulation.log_dir.empty()
