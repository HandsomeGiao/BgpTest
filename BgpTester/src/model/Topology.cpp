...
bool isIpv4Cidr(const QString &value) {
  ...
  return prefixOk && prefixLength >= 0 && prefixLength <= 32 &&
         isIpv4(value.first(slash));
}

quint32 jsonUint(const QJsonObject &object, const QString &key,
                 quint32 fallback) {
...
int jsonNonNegativeInt(const QJsonObject &object, const QString &key,
                       int fallback) {
  const auto value = object.value(key);
  if (!value.isDouble()) {
    return fallback;
...
QString toString(SessionType type) {
  return type == SessionType::Ebgp ? QStringLiteral("ebgp")
                                   : QStringLiteral("ibgp");
}
...
QJsonObject Topology::toJson() const {
  ...
  for (const auto &link : links) {
    ...
    const auto session = aIt->asn == bIt->asn ? QStringLiteral("ibgp")
                                              : QStringLiteral("ebgp");
  ...
  for (auto it = routers.cbegin(); it != routers.cend(); ++it) {
    ...
    routerArray.append(QJsonObject{
        ...
        {QStringLiteral("cluster_id"),
         router.clusterId.isEmpty() ? router.routerId : router.clusterId},
        {QStringLiteral("originated_prefixes"),
         stringArray(router.originatedPrefixes)},
  ...
  return {{QStringLiteral("simulation"), simulationObject},
          {QStringLiteral("routers"), routerArray},
          {QStringLiteral("links"), linkArray}};
}

std::optional<Topology> Topology::fromJson(const QJsonObject &object,
                                           QString *error) {
  Topology topology;
  const auto simulationObject = object.value(QStringLiteral("simulation"))
                                    .toObject();
  topology.simulation.name =
      simulationObject.value(QStringLiteral("name"))
          .toString(QStringLiteral("bgp-lab"));
  topology.simulation.logDirectory =
      simulationObject.value(QStringLiteral("log_dir"))
          .toString(QStringLiteral("tmp"));
  topology.simulation.convergenceQuietMs = jsonNonNegativeInt(
      simulationObject, QStringLiteral("convergence_quiet_ms"), 1000);
  ...
  for (const auto &value : routerValues) {
    ...
    RouterConfig router;
    router.id = entry.value(QStringLiteral("id"))
                    .toString(QStringLiteral("R%1").arg(index + 1))
                    .trimmed();
    ...
    router.routerId = entry.value(QStringLiteral("router_id"))
                          .toString(routerIdFromIndex(index + 1))
                          .trimmed();
    router.asn = jsonUint(entry, QStringLiteral("asn"), 65000);
    router.clusterId = entry.value(QStringLiteral("cluster_id"))
                           .toString(router.routerId)
                           .trimmed();
    router.originatedPrefixes =
        readStringArray(entry.value(QStringLiteral("originated_prefixes")));
    const auto position = entry.value(QStringLiteral("position")).toObject();
    router.position =
        QPointF(position.value(QStringLiteral("x"))
  ...
  for (const auto &value : linkValues) {
    ...
    link.rrClientFromA =
        entry.value(QStringLiteral("rr_client_from_a")).toBool(false);
    link.rrClientFromB =
        entry.value(QStringLiteral("rr_client_from_b")).toBool(false);
    link.mraiMsFromA =
        jsonNonNegativeInt(entry, QStringLiteral("mrai_ms_from_a"), 0);
    link.mraiMsFromB =
        jsonNonNegativeInt(entry, QStringLiteral("mrai_ms_from_b"), 0);
  ...
  for (const auto &routerObject : originalRouters) {
    ...
    for (const auto &neighborValue :
         routerObject.value(QStringLiteral("neighbors")).toArray()) {
      const auto neighborObject = neighborValue.toObject();
      const auto peerId =
          neighborObject.value(QStringLiteral("id")).toString().trimmed();
      if (!topology.routers.contains(routerId) ||
          !topology.routers.contains(peerId) || routerId == peerId) {
        continue;
      }
      ...
      if (!link) {
        ...
        generated.b = peerId;
        generated.enabled =
            neighborObject.value(QStringLiteral("enabled")).toBool(true);
        topology.links.append(generated);
        link = &topology.links.last();
      }
      const auto rr =
          neighborObject.value(QStringLiteral("rr_client")).toBool(false);
      const auto mrai = jsonNonNegativeInt(neighborObject,
                                           QStringLiteral("mrai_ms"), 0);
      const auto enabled =
          neighborObject.value(QStringLiteral("enabled")).toBool(true);
...
std::optional<Topology> Topology::load(const QString &path, QString *error) {
  ...
  if (document.isNull() || !document.isObject()) {
    if (error) {
      *error = QStringLiteral("JSON 解析失败（偏移 %1）：%2")
...
QStringList Topology::validate() const {
  ...
  for (auto it = routers.cbegin(); it != routers.cend(); ++it) {
    ...
    if (router.id != it.key()) {
      problems.append(QStringLiteral("路由器映射键与 ID 不一致：%1 / %2")
                          .arg(it.key(), router.id));
    }
    if (!isIpv4(router.routerId, false)) {
      problems.append(
          QStringLiteral("%1 的 BGP Router ID 无效：%2")
              .arg(router.id, router.routerId));
    } else if (bgpRouterIds.contains(router.routerId)) {
      problems.append(
          QStringLiteral("BGP Router ID 重复：%1").arg(router.routerId));
    }
    ...
    for (const auto &prefix : router.originatedPrefixes) {
      if (!isIpv4Cidr(prefix)) {
        problems.append(QStringLiteral("%1 的前缀无效：%2")
                            .arg(router.id, prefix));
      }
    }
  }
  ...
  for (const auto &link : links) {
    if (link.a == link.b) {
      problems.append(QStringLiteral("链路不能连接路由器自身：%1")
                          .arg(link.a));
    }
    if (!routers.contains(link.a) || !routers.contains(link.b)) {
      problems.append(QStringLiteral("链路端点不存在：%1 - %2")
                          .arg(link.a, link.b));
    }
    const auto key = edgeKey(link.a, link.b);
    if (edges.contains(key)) {
      problems.append(
          QStringLiteral("链路重复：%1 - %2").arg(link.a, link.b));
    }
    edges.insert(key);
    if (link.delayMs < 0 || link.mraiMsFromA < 0 || link.mraiMsFromB < 0) {
      problems.append(QStringLiteral("链路 %1 - %2 的延迟/MRAI 不能为负数")
                          .arg(link.a, link.b));
    }
  }
  return problems;
...
QVector<NeighborConfig> Topology::neighborsFor(const QString &routerId) const {
  ...
  for (const auto &link : links) {
    ...
    result.append(NeighborConfig{
        ...
        .sessionType = localIt->asn == peerIt->asn ? SessionType::Ibgp
                                                   : SessionType::Ebgp,
...
const LinkConfig *Topology::findLink(const QString &a, const QString &b) const {
  ...
  const auto it = std::find_if(links.cbegin(), links.cend(), [&](const auto &l) {
    return edgeKey(l.a, l.b) == key;
  });
