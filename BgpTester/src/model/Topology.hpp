...
public:
  ...
  [[nodiscard]] static std::optional<Topology>
  fromJson(const QJsonObject &object, QString *error = nullptr);
  [[nodiscard]] static std::optional<Topology>
  load(const QString &path, QString *error = nullptr);
  [[nodiscard]] bool save(const QString &path, QString *error = nullptr) const;

  [[nodiscard]] QStringList validate() const;
  [[nodiscard]] QVector<NeighborConfig>
  neighborsFor(const QString &routerId) const;
  [[nodiscard]] const LinkConfig *findLink(const QString &a,
