...
QVariant EventTableModel::data(const QModelIndex &index, int role) const {
  ...
  if (role == Qt::TextAlignmentRole) {
    return index.column() == Prefixes || index.column() == Withdrawn ||
                   index.column() == AsPath
  ...
  case LocalPref:
    return event.localPref ? QVariant::fromValue(*event.localPref)
                           : QVariant{};
  case Med:
...
QVariant EventTableModel::headerData(int section, Qt::Orientation orientation,
  ...
  static const QStringList headers{
      QStringLiteral("ID"),       QStringLiteral("时间"),
      QStringLiteral("事件"),     QStringLiteral("路由器"),
      QStringLiteral("来源"),     QStringLiteral("目的"),
      QStringLiteral("来源 AS"),  QStringLiteral("目的 AS"),
      QStringLiteral("报文"),     QStringLiteral("动作"),
      QStringLiteral("序号"),     QStringLiteral("前缀"),
      QStringLiteral("撤销"),     QStringLiteral("NEXT_HOP"),
