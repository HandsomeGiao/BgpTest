...
public:
  ...
  [[nodiscard]] QVector<PeerSnapshot>
  peerSnapshots(const QString &routerId) const;
  [[nodiscard]] QVector<RouterSnapshot> routerSnapshots() const;
  [[nodiscard]] BestPathUpdate bestPath(const QString &routerId,
...
signals:
  ...
  void peersSnapshotReady(QString routerId,
...
private:
  ...
  enum class ScheduledKind { DeliverMessages, FlushMrai };
  ...
  void scheduleMessages(const QString &from, const QString &to,
                        QVector<BgpMessage> messages, int extraDelayMs = 0);
  void scheduleMraiFlush(const QString &from, const QString &to,
                         qint64 dueAt);
  void deliverMessages(const ScheduledEvent &event);
  void flushMrai(const QString &from, const QString &to);
  [[nodiscard]] bool messageDeliverable(const QString &from,
  ...
  void handleUpdateBatch(const QString &receiver, const QString &sender,
  ...
  void runDecision(const QString &routerId,
  ...
  void queueAdvertisement(const QString &routerId, const QString &peerId,
                          ...
                          const std::optional<RouteEntry> &route);
  [[nodiscard]] BgpMessage
  makeUpdateMessage(const QString &from, const QString &to,
                    const QString &prefix,
                    const std::optional<RouteEntry> &route,
                    quint64 generation) const;

  void recordMessage(const BgpMessage &message);
  void recordTopologyEvent(const QString &name,
                           QMap<QString, QString> details = {});
  [[nodiscard]] bool hasRouteReflectorClients(
      const RouterRuntime &router) const;

  Topology topology_;
  QMap<QString, RouterRuntime> routers_;
  QMap<QString, LinkConfig> links_;
  std::priority_queue<ScheduledEvent, std::vector<ScheduledEvent>, LaterEvent>
      events_;
