...
public:
  ...
  [[nodiscard]] bool beginRun(const SimulationSettings &settings,
                              QString *error = nullptr);
  void endRun();
  void flush();
  void clearRecent();

  [[nodiscard]] bool isOpen() const { return database_.isOpen(); }
  [[nodiscard]] QString runDirectory() const { return runDirectory_; }
  [[nodiscard]] QString logFilePath() const { return logFile_.fileName(); }
  [[nodiscard]] QString databasePath() const { return databasePath_; }
  [[nodiscard]] const QVector<SimulationEvent> &recentEvents() const {
    return recentEvents_;
  }

  [[nodiscard]] static QVector<SimulationEvent>
  readDatabase(const QString &path, int limit, QString *error = nullptr);
...
private:
  ...
  bool insertEvent(const SimulationEvent &event, const QByteArray &rawJson,
                   QString *error);
