...
{
public:
    explicit ConfigurableExportRouterNode(RouterNodeContext context, QObject* parent = nullptr) : RouterNode(std::move(context), parent)
    {
    }
    ...
    QStringList validateConfiguration() const override
    {
        ...
        if (settings.contains(QStringLiteral("export_routes")) && !settings.value(QStringLiteral("export_routes")).isBool())
        {
        ...
        if (settings.contains(QStringLiteral("local_preference")))
        {
            ...
            if (!value.isDouble() || number < 0.0 || number > std::numeric_limits<quint32>::max() || std::floor(number) != number)
            {
                problems.append(QStringLiteral("local_preference 必须是 0..4294967295 的整数"));
            }
        }
        return problems;
    }
    ...
    std::optional<RouteEntry> selectBestRoute(const QString&, const QVector<RouteEntry>& candidates,
                                              const std::optional<RouteEntry>&) override
    {
        ...
        return *std::min_element(candidates.cbegin(), candidates.cend(),
                                 ...
                                 {
                                     return std::tuple(!lhs.localOrigin, std::numeric_limits<quint32>::max() - lhs.attributes.localPref,
                                                       lhs.attributes.asPath.size(), lhs.learnedFrom) <
                                            std::tuple(!rhs.localOrigin, std::numeric_limits<quint32>::max() - rhs.attributes.localPref,
    ...
    {
        if (!context().config.pluginSettings.value(QStringLiteral("export_routes")).toBool(false) || route.learnedFrom == toPeer.id)
        {
            return std::nullopt;
        }
...
RouterPluginMetadata ConfigurableExportRouterPlugin::metadata() const
{
    return RouterPluginMetadata{
        ...
        .description = QStringLiteral("演示自定义选路与出口控制；export_routes 决定是否发布路由。"),
        .apiVersion = RouterPluginApiVersion,
        .defaultSettings = QJsonObject{{QStringLiteral("export_routes"), false}, {QStringLiteral("local_preference"), 100}},
    };
}

RouterNode* ConfigurableExportRouterPlugin::createRouterNode(const RouterNodeContext& context, QObject* parent, QString* error)
{
    if (error)
    {
        error->clear();
...
BGPTESTER_REGISTER_ROUTER_PLUGIN(bgptester::ConfigurableExportRouterPlugin)
